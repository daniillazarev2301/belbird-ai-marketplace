...
import { SlidersHorizontal, ChevronDown, X, Grid3X3, List, Check, ArrowUpDown } from "lucide-react";
...
export function HorizontalFilters({
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-[calc(56px+env(safe-area-inset-bottom))] left-0 right-0 z-40 bg-background border-t">
        <div className="flex items-center">
          {/* Sort Sheet */}
          <Sheet>
            <SheetTrigger asChild>
              <button className="flex-1 flex items-center justify-center gap-2 py-3 border-r active:bg-muted/50 transition-colors">
          ...
          <Sheet open={allFiltersOpen} onOpenChange={setAllFiltersOpen}>
            <SheetTrigger asChild>
              <button className="flex-1 flex items-center justify-center gap-2 py-3 active:bg-muted/50 transition-colors">
                <SlidersHorizontal className="h-4 w-4" />
                <span className="text-sm font-medium">Фильтры</span>
                {activeFiltersCount > 0 && (
                  <Badge className="h-5 px-1.5 min-w-[20px] text-xs">
                    {activeFiltersCount}
                  </Badge>
                )}
              </button>
            ...
            <SheetContent side="bottom" className="h-[85vh] rounded-t-2xl p-0">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {brands.map((brand) => (
                          <label
                            key={brand.id}
                            className="flex items-center gap-3 cursor-pointer py-3 px-2 rounded-xl hover:bg-muted transition-colors"
                          >
                            <Checkbox
                              checked={selectedBrands.includes(brand.name)}
                              onCheckedChange={() => onBrandToggle(brand.name)}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-background border-t">
                <Button 
                  onClick={() => { applyPriceFilter(); setAllFiltersOpen(false); }} 
